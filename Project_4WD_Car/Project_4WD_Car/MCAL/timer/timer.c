...
EN_TimerError_t TIMER_timer0Delay(float64 f64_a_timeDelay) {
   float64 f64_v_Timer0_OVFNumber, f64_v_Timer0_OVFtime;
   if (f64_a_timeDelay < INITIAL_VALUE_0) {
      return TIMER_INVALID_DELAY;
   }
   f64_v_Timer0_OVFtime = f64_a_timeDelay / NUMBER_OF_OVERFLOWS;
   f64_v_Timer0_OVFNumber = ceil(f64_v_Timer0_OVFtime / FREQUENCY_OF_OVERFLOWS);
   ...
   if (TIMER_timer0_OVF_Number(f64_v_Timer0_OVFNumber) != TIMER_OK) {
      return TIMER_INVALID_DELAY;
   }
   return TIMER_OK;
}
...
EN_TimerError_t TIMER_timer0PWM_mode(float32 f32_a_pwmValue) {
   if (f32_a_pwmValue < INITIAL_VALUE_0) {
...
EN_TimerError_t TIMER_timer2SetInitial(uint16_t u8_a_timer2IntialValue) {
...
void test_timer(void) {
   /* test case 1.. returns TIMER_INVALID_MODE */
   TIMER_timer0InitMode(timerFailTest);

   /* test case 2.. returns TIMER_INVALID_PRESCALAR */
   TIMER_timer0Prescalar(timerFailTest);

   /* test case 3.. returns TIMER_INVALID_SETINIT*/
   TIMER_timer0SetInitial(FAIL_MAX);

   /* test case 4.. returns TIMER_INVALID_DELAY*/
   TIMER_timer0Delay(INVALID_NEG_VAL);

   /* test case 5.. returns TIMER_INVALID_T0PWM*/
   TIMER_timer0PWM_mode(INVALID_NEG_VAL);

   /* test case 6.. returns TIMER_INVALID_MODE */
   TIMER_timer2InitMode(timerFailTest);

   /* test case 7.. returns TIMER_INVALID_PRESCALAR */
   TIMER_timer2Prescalar(timerFailTest);
